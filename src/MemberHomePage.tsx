...
type Surface = "menu" | "notifications" | "language" | "status" | "updates" | null;
...
export function MemberHomePage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => { if (!document.hidden) void loadTickets(); }, 12_000);
  ...
  const renderSurface = () => {
    ...
    return <aside className={`member-command-surface ${surface}`}>
      ...
      {surface === "updates" && <><span>GÜNCELLEME NOTLARI</span><h2>SW Identity v{SW_IDENTITY_VERSION}</h2><article><b><i /> Destek posta köprüsü</b><p>Talep numaraları ve e-posta yanıtlarının güvenli biçimde konuşmaya dönmesi etkin.</p></article><article><b><i /> Abonelik veri akışı</b><p>SW Create ve Play Streamers plan kataloğu, üyelik durumu ve ürün erişimi tek güvenli kaynaktan okunuyor.</p></article><article><b><i /> Güvenli işlem doğrulaması</b><p>Önemli hesap işlemleri Authenticator veya e-posta koduyla korunuyor.</p></article></>}
      {surface === "menu" && <><span>SW MENÜ</span><h2>{displayName}</h2><a href="/center/">Hesabım <b>→</b></a><button type="button" className="surface-link" onClick={() => setSurface("updates")}>Güncelleme notları <b>→</b></button><a href="/plans/">Planlar <b>→</b></a><button type="button" className="surface-link" onClick={() => void openSupport()}>Destek taleplerim <b>→</b></button><a href="/privacy/">Gizlilik politikası <b>→</b></a><button type="button" className="member-menu-logout" onClick={() => { setSurface(null); setLogoutConfirm(true); }}>Çıkış yap</button></>}
    </aside>;
  };
  ...
  return <main className="member-shell member-home-shell">
    ...
    <section className="member-content member-home-content"><div className="member-welcome"><p>{t.home} · SW IDENTITY BAĞLI</p><h1>{t.hello},<br />{displayName.toLocaleUpperCase(locale === "tr" ? "tr-TR" : "en-US")}.</h1></div><div className="member-dashboard">
      ...
      <section className="member-whats-new"><div><span>SW SİNYALİ / v{SW_IDENTITY_VERSION}</span><h2>{t.news}</h2><p>SW Identity artık destek yanıtlarıyla abonelik ve ürün erişimi verisini aynı doğrulanmış veri akışında koruyor.</p></div><div className="member-release-list"><article><b>01</b><span>Talep numarası ve posta köprüsü<small>Her destek talebi görünür bir numara alır; e-posta yanıtı doğru konuşmaya döner.</small></span></article><article><b>02</b><span>Plan ve abonelik ağı<small>SW Create ve Play Streamers planları güvenli katalogdan okunur.</small></span></article><article><b>03</b><span>Güvenli işlem onayı<small>E-posta, şifre ve hesap silme adımları güçlü doğrulamadan geçer.</small></span></article></div></section>
