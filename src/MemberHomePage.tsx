...
type Surface = "menu" | "notifications" | "language" | "status" | "dashboard" | "updates" | "plans" | null;
...
export function MemberHomePage() {
  ...
  async function submitTicket(event: FormEvent) {
    ...
    try {
      ...
      setSupportSubject(""); setSupportMessage(""); setSupportNotice("Talebin SW destek ağına ulaştı.");
  ...
  async function replyTicket(event: FormEvent) {
    ...
    try {
      await apiRequest(`/api/support/tickets/${currentTicket.id}/messages`, { method: "POST", body: JSON.stringify({ message: supportMessage }) });
  ...
  const renderSurface = () => {
    ...
    return <aside className={`member-command-surface ${surface}`}>
      ...
      {surface === "dashboard" && <><span>SW DASHBOARD</span><h2>Plan ve erişim panelin</h2><div className="dashboard-plan"><small>MEVCUT PLAN</small><strong>SW FREE</strong><p>{memberSince} tarihinden beri etkin</p></div><div className="dashboard-entitlements">{account.entitlements.map((item) => <article key={item.slug}><span><b>{item.product}</b><small>{item.slug}</small></span><strong>{item.tier.toUpperCase()}</strong></article>)}</div><button type="button" className="surface-action" onClick={() => setSurface("plans")}>Tüm planları incele →</button></>}
  ...
  return <main className="member-shell member-home-shell">
    ...
    <header className="member-topbar member-command-bar" ref={commandRef}>
      ...
      <div className="member-home-tools">
        <button type="button" className={`member-dashboard-button ${surface === "dashboard" ? "active" : ""}`} onClick={() => toggle("dashboard")}>{t.dashboard}<b>▦</b></button>
    ...
    {supportOpen && <aside className="member-support-drawer" ref={supportRef} aria-label="SW destek merkezi"><header><div><span>SW DESTEK AĞI</span><h2>Nasıl yardımcı olabiliriz?</h2></div><button type="button" onClick={() => setSupportOpen(false)}>×</button></header><div className="support-layout"><nav><button type="button" className={!selectedTicket ? "active" : ""} onClick={() => { setSelectedTicket(null); setSupportMessage(""); }}>+ Yeni destek talebi</button>{tickets.map((ticket) => <button type="button" key={ticket.id} className={selectedTicket === ticket.id ? "active" : ""} onClick={() => { setSelectedTicket(ticket.id); setSupportMessage(""); }}><span>{ticket.subject}</span><small>{ticket.status === "answered" ? "YANITLANDI" : "YANIT BEKLİYOR"}</small></button>)}</nav><section>{currentTicket ? <><div className="support-ticket-head"><span>{currentTicket.category}</span><h3>{currentTicket.subject}</h3><small>{timeLabel(currentTicket.updatedAt, locale)}</small></div><div className="support-conversation">{currentTicket.messages.map((message) => <article key={message.id} className={message.sender}><b>{message.sender === "support" ? "SW DESTEK" : "SEN"}</b><p>{message.body}</p><small>{timeLabel(message.createdAt, locale)}</small></article>)}</div><form onSubmit={replyTicket}><textarea value={supportMessage} onChange={(event) => setSupportMessage(event.target.value)} minLength={4} maxLength={2000} placeholder="Yanıtını yaz…" required /><button disabled={busy}>Mesajı gönder</button></form></> : <form className="support-new-form" onSubmit={submitTicket}><label>Kategori<select value={supportCategory} onChange={(event) => setSupportCategory(event.target.value)}><option value="technical">Teknik sorun</option><option value="account">Hesap ve güvenlik</option><option value="plans">Planlar ve ürünler</option><option value="feedback">Öneri</option></select></label><label>Konu<input value={supportSubject} onChange={(event) => setSupportSubject(event.target.value)} minLength={4} maxLength={100} required /></label><label>Mesaj<textarea value={supportMessage} onChange={(event) => setSupportMessage(event.target.value)} minLength={10} maxLength={2000} required /></label><button disabled={busy}>{busy ? "GÖNDERİLİYOR…" : "TALEBİ GÖNDER"}</button>{supportNotice && <p>{supportNotice}</p>}</form>}</section></div></aside>}
