...
import { savedSwLanguage, SW_LANGUAGES, swLocaleTag, type SwLanguage } from "./languages";
import { rememberSwAccount } from "./rememberedAccounts";
import { SW_IDENTITY_VERSION } from "./security";

type Surface = "menu" | "notifications" | "language" | "status" | null;
...
function timeLabel(timestamp: number, locale: SwLanguage) {
  return new Intl.DateTimeFormat(swLocaleTag(locale), { dateStyle: "medium", timeStyle: "short" }).format(timestamp * 1000);
}
...
export function MemberHomePage() {
  ...
  const primarySubscription = account?.subscriptions.find((item) => item.productId === "sw-create" && item.status === "active") ?? account?.subscriptions.find((item) => item.status === "active");
  const membershipLabel = primarySubscription?.tier === "product-pro" || primarySubscription?.planId.includes("product-pro")
    ? "PRODUCT PRO"
    : primarySubscription?.planName || "FREE";
  ...
  useEffect(() => {
    ...
    const refresh = () => { if (!document.hidden) void loadTickets(); };
    refresh();
    const timer = window.setInterval(refresh, 3_000);
    window.addEventListener("focus", refresh);
    document.addEventListener("visibilitychange", refresh);
    return () => { window.clearInterval(timer); window.removeEventListener("focus", refresh); document.removeEventListener("visibilitychange", refresh); };
  }, [supportOpen, loadTickets]);
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(() => void syncNotifications(), 5_000);
  ...
  const runDiagnostics = useCallback(async () => {
    const controlCount = document.querySelectorAll(".member-home-tools button, .member-home-tools a").length;
    const duplicateIds = Array.from(document.querySelectorAll<HTMLElement>("[id]")).filter((element, index, elements) => elements.findIndex(candidate => candidate.id === element.id) !== index);
    const brokenImages = Array.from(document.images).filter(image => image.complete && image.naturalWidth === 0);
    const unlabeledControls = Array.from(document.querySelectorAll<HTMLElement>("button, a")).filter(control => {
    ...
    const unsafeExternalLinks = Array.from(document.querySelectorAll<HTMLAnchorElement>('a[target="_blank"]')).filter(link => !link.rel.split(/\s+/).includes("noreferrer"));
    unsafeExternalLinks.forEach(link => { link.rel = `${link.rel} noopener noreferrer`.trim(); });
    unlabeledControls.forEach((control, index) => control.setAttribute("aria-label", `SW arayüz kontrolü ${index + 1}`));
    brokenImages.forEach(image => {
      if (image.dataset.swRetried) return;
      image.dataset.swRetried = "true";
      const next = new URL(image.currentSrc || image.src, window.location.href);
      next.searchParams.set("sw-retry", String(Date.now()));
    ...
    document.documentElement.classList.toggle("sw-layout-guard", hasHorizontalOverflow);
    const results: Diagnostic[] = [
      ...
      { label: "Yerleşim ve taşma", detail: hasHorizontalOverflow ? "Yatay taşma algılandı; güvenli yerleşim koruması uygulandı." : "Görünür alanda yatay taşma yok.", state: hasHorizontalOverflow ? "warning" : "ok" },
      { label: "Görsel bütünlüğü", detail: brokenImages.length ? `${brokenImages.length} görsel yeniden yükleniyor.` : "Görünür görseller eksiksiz yüklendi.", state: brokenImages.length ? "warning" : "ok" },
      { label: "Erişilebilir kontroller", detail: unlabeledControls.length ? `${unlabeledControls.length} etiketsiz kontrole güvenli ad eklendi.` : "Görünür kontrollerin adları tamam.", state: unlabeledControls.length ? "warning" : "ok" },
      { label: "Arayüz kimlikleri", detail: duplicateIds.length ? `${duplicateIds.length} yinelenen kimlik bulundu; geliştirici kaydı oluşturuldu.` : "Yinelenen arayüz kimliği yok.", state: duplicateIds.length ? "warning" : "ok" },
      { label: "Dil kapsamı", detail: SW_LANGUAGES.length === 8 ? "Sekiz dil paketi ve gerçek bayrak varlıkları hazır." : "Dil paketlerinden biri eksik.", state: SW_LANGUAGES.length === 8 ? "ok" : "error" },
      { label: "Dış bağlantı güvenliği", detail: unsafeExternalLinks.length ? `${unsafeExternalLinks.length} bağlantının güvenlik niteliği onarıldı.` : "Yeni sekme bağlantıları güvenli.", state: "ok" },
    ];
    try {
      const response = await fetch(`${API_BASE}/api/health`, { cache: "no-store" });
      const health = await response.json() as { ok?: boolean; version?: string; dataFlow?: string; twoFactor?: string; mail?: string };
      ...
      results.push({ label: "Güvenli e-posta köprüsü", detail: health.mail === "available" ? "Kod ve destek e-postası veri akışı hazır." : "E-posta secret veya inbound alan adı eksik.", state: health.mail === "available" ? "ok" : "warning" });
  ...
  async function openNotification(notification: SwNotification) {
    ...
    await apiRequest("/api/notifications/read", { method: "POST", body: JSON.stringify({ ids: [notification.id] }) }).catch(() => undefined);
    setSurface(null);
    if (notification.target === "updates") window.location.assign("/updates/");
    else {
      setSupportOpen(true);
  ...
  async function submitTicket(event: FormEvent) {
    ...
    try {
      ...
      await apiRequest(`/api/support/tickets/${data.ticket.id}/send`, { method: "POST", body: "{}" });
      setSupportSubject(""); setSupportMessage(""); setSupportNotice(`Talebin SW destek ağına ulaştı. Talep numaran: ${data.ticket.ticketNumber}`);
  ...
  const memberSince = new Intl.DateTimeFormat(swLocaleTag(locale), { month: "long", year: "numeric" }).format(account.user.createdAt * 1000);
  ...
  const renderSurface = () => {
    ...
    return <aside className={`member-command-surface ${surface}`}>
      ...
      {surface === "notifications" && <><span>BİLDİRİM MERKEZİ</span><h2>{unreadCount ? `${unreadCount} yeni sinyal` : "Yeni sinyal yok"}</h2><div className="notification-list">{notifications.length ? notifications.map((notification) => <button type="button" key={notification.id} className={notification.read ? "notification-card read" : "notification-card"} onClick={() => void openNotification(notification)}><i /><span><b>{notification.title}</b><p>{notification.body}</p><small>{timeLabel(notification.createdAt, locale)} · Aç</small></span><strong>→</strong></button>) : <p className="member-empty">Bildirim ağı sessiz.</p>}</div></>}
      {surface === "language" && <><span>DİL SEÇİMİ</span><h2>Arayüz dili</h2><div className="member-language-menu">{SW_LANGUAGES.map(([code, flag, label]) => <button type="button" key={code} className={locale === code ? "active" : ""} onClick={() => { localStorage.setItem("sw-language", code); setLocale(code); setSurface(null); window.location.reload(); }}><img src={flag} alt="" /><span>{label}</span><b>{code.toUpperCase()}</b></button>)}</div></>}
      {surface === "status" && <><span>SİSTEM DURUMU · SW BOT</span><h2>{statusSummary}</h2><p className="sw-bot-intro">SW Bot; bağlantı, SW Identity, sürüm, veri akışı, görseller, erişilebilirlik, dil kapsamı ve responsive yerleşimi birlikte denetler; güvenli arayüz düzeltmelerini otomatik uygular.</p><div className="diagnostic-list">{diagnostics.map((item) => <article key={item.label} className={item.state}><b><i />{item.label}</b><p>{item.detail}</p></article>)}</div><button type="button" className="diagnostic-refresh" onClick={() => void runDiagnostics()}>Denetle ve düzelt</button></>}
      {surface === "menu" && <><span>SW MENÜ</span><h2>{displayName}</h2><a href="/center/">Hesabım <b>→</b></a><a href="/updates/">Güncelleme notları <b>→</b></a><a href="/plans/">Planlar <b>→</b></a><button type="button" className="surface-link" onClick={() => void openSupport()}>Destek taleplerim <b>→</b></button><a href="/privacy/">Gizlilik politikası <b>→</b></a><button type="button" className="member-menu-logout" onClick={() => { setSurface(null); setLogoutConfirm(true); }}>Çıkış yap</button></>}
    </aside>;
  };
  ...
  return <main className="member-shell member-home-shell">
    ...
    <header className="member-topbar member-command-bar" ref={commandRef}>
      <a href="/home/" className="member-brand"><img src="/brand/swcreate-logo-128.webp" alt="" /><span>SW CREATE<small>{t.home}</small></span></a>
      <div className="member-home-tools">
        <a className="member-dashboard-button" href="/dashboard/">{t.dashboard}<b>▦</b></a>
        <button type="button" className={surface === "notifications" ? "active" : ""} onClick={() => toggle("notifications")} title={t.notifications} aria-label={t.notifications}><span className="notification-glyph" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M6.4 16.1V10a5.6 5.6 0 0 1 11.2 0v6.1l1.7 2.1H4.7l1.7-2.1Z" /><path d="M9.7 20.1c.5.6 1.3.9 2.3.9s1.8-.3 2.3-.9" /><path className="signal" d="M18.2 5.1c1.1.8 1.8 1.9 2.1 3.2" /></svg></span>{unreadCount > 0 && <em>{unreadCount}</em>}</button>
    ...
    <section className="member-content member-home-content"><div className="member-welcome"><p>{t.home} · SW IDENTITY BAĞLI</p><h1>{t.hello.toLocaleLowerCase(locale === "tr" ? "tr-TR" : "en-US")}, <em>{displayName}.</em></h1></div><div className="member-dashboard">
      <section className="member-briefing"><div><span>SW AĞI / BUGÜN</span><h2>Yörüngen hazır.</h2><p>Burası sana ait kullanıcı ana sayfası. Ürünlerin, planların, bildirimlerin ve destek taleplerin tanıtım sitesinden bağımsız çalışır.</p></div><div className="member-orbit-mark" aria-hidden="true"><span><img src="/brand/swcreate-logo-128.webp" alt="" /></span><i /><i /><i /></div></section>
      <section className="member-whats-new"><div><span>SW SİNYALİ / v{SW_IDENTITY_VERSION}</span><h2>{t.news}</h2><p>SW Identity artık hızlı hesap seçimini, posta eklerini ve sağlayıcı bağlantılarını doğrulanmış veri akışında koruyor.</p><a className="member-updates-arrow" href="/updates/" aria-label="Güncelleme notlarını aç">GÜNCELLEME NOTLARINI AÇ <b>→</b></a></div><div className="member-release-list without-numbers"><article><span>Canlı destek posta köprüsü<small>E-posta yanıtı ve güvenli ekleri açık konuşmaya sayfa yenilemeden döner.</small></span></article><article><span>Güvenli hesap seçimi<small>Parola saklanmadan, bu cihaza özel dönen anahtarla kayıtlı hesaba girilir.</small></span></article><article><span>Kimlik bağlantı ağı<small>Google ve Kick hesapları SW Identity merkezinden güvenle bağlanır.</small></span></article></div></section>
      <div className="account-summary-grid"><article><span>SW KİMLİĞİ</span><strong>{account.user.id.slice(0, 8).toUpperCase()}</strong><small>Merkezi hesap numaran</small></article><article><span>ÜRÜN ERİŞİMİ</span><strong>{String(account.entitlements.length)}</strong><small>Etkin ürün bağlantısı</small></article><article><span>ÜYELİK</span><strong>{membershipLabel}</strong><small>{memberSince} tarihinden beri</small></article></div>
      <div className="member-dashboard-grid"><section className="member-products"><div className="member-section-head"><div><span>ÜRÜN AĞI</span><h2>{t.products}</h2></div><b>{account.entitlements.length} BAĞLANTI</b></div>{account.entitlements.length > 0 ? account.entitlements.map((item) => <article key={item.slug}><div><i /><span>{item.product}<small>{item.slug}</small></span></div><b>{item.tier.toUpperCase()}</b></article>) : <p className="member-empty">İlk ürün erişimin hesabına eklendiğinde burada görünecek.</p>}</section><aside className="member-quick-panel"><span>HIZLI GEÇİŞLER</span><h2>Ürün ağı</h2><a href="https://pstreamers.com" target="_blank" rel="noreferrer"><i>PS</i><span>Play Streamers<small>Yayıncı alanını aç</small></span><b>↗</b></a><a href="https://apps.microsoft.com/detail/9NWZ0TF5K999" target="_blank" rel="noreferrer"><i>APP</i><span>Play Streamers App<small>Windows uygulamasını Microsoft Store'da aç</small></span><b>↗</b></a><a href={playConnectUrl()} target="_blank" rel="noreferrer"><i>PC</i><span>Play Connect<small>Tarayıcına uygun mağaza sayfasını aç</small></span><b>↗</b></a><a href="/dashboard/"><i>SW</i><span>SW Dashboard<small>Plan ve erişimlerini görüntüle</small></span><b>→</b></a></aside></div>
    ...
    {supportOpen && <aside className="member-support-drawer" ref={supportRef} aria-label="SW destek merkezi">
      ...
      <div className="support-layout"><nav><button type="button" className={!selectedTicket ? "active" : ""} onClick={() => { setSelectedTicket(null); setSupportMessage(""); setSupportFiles([]); }}>+ Yeni destek talebi</button>{tickets.map((ticket) => <button type="button" key={ticket.id} className={selectedTicket === ticket.id ? "active" : ""} onClick={() => { setSelectedTicket(ticket.id); setSupportMessage(""); setSupportFiles([]); }}><span>{ticket.subject}</span><small>{ticket.ticketNumber} · {ticket.status === "answered" ? "YANITLANDI" : "YANIT BEKLİYOR"}</small></button>)}</nav>
        <section>{currentTicket ? <><div className="support-ticket-head"><span>{currentTicket.category} · {currentTicket.ticketNumber}</span><h3>{currentTicket.subject}</h3><small>{timeLabel(currentTicket.updatedAt, locale)} · {currentTicket.mailStatus === "sent" ? "E-POSTAYA İLETİLDİ" : currentTicket.mailStatus === "failed" ? "E-POSTA İLETİMİ BAŞARISIZ" : "E-POSTA HAZIRLANIYOR"}</small></div><div className="support-conversation">{currentTicket.messages.map((message) => <article key={message.id} className={message.sender}><b>{message.sender === "support" ? "SW DESTEK" : "SEN"}</b><p>{message.body}</p>{message.attachments.length > 0 && <div className="support-attachments">{message.attachments.map((attachment) => <a key={attachment.id} href={`${API_BASE}${attachment.url}?download=1`} download={attachment.fileName} target="_blank" rel="noreferrer">{attachment.mimeType.startsWith("image/") && <img src={`${API_BASE}${attachment.url}`} alt="" />}<span>{attachment.fileName}<small>{Math.ceil(attachment.size / 1024)} KB · İNDİR</small></span></a>)}</div>}<small>{timeLabel(message.createdAt, locale)}</small></article>)}</div><p className="support-reply-note">Yanıtlar ve dosyalar SW Destek’in e-posta yanıtından güvenli şekilde bu konuşmaya düşer.</p></> : <form className="support-new-form" onSubmit={submitTicket}><label>Kategori<select value={supportCategory} onChange={(event) => setSupportCategory(event.target.value)}><option value="technical">Teknik sorun</option><option value="account">Hesap ve güvenlik</option><option value="plans">Planlar ve ürünler</option><option value="feedback">Öneri</option><option value="other">Diğer</option></select></label><label>Konu<input value={supportSubject} onChange={(event) => setSupportSubject(event.target.value)} minLength={4} maxLength={100} required /></label><label>Mesaj<textarea value={supportMessage} onChange={(event) => setSupportMessage(event.target.value)} minLength={10} maxLength={2000} required /></label><FilePicker files={supportFiles} onFiles={chooseSupportFiles} onRemove={(index) => setSupportFiles((items) => items.filter((_, itemIndex) => itemIndex !== index))} /><button disabled={busy}>{busy ? "GÖNDERİLİYOR…" : "TALEBİ VE DOSYALARI GÖNDER"}</button>{supportNotice && <p>{supportNotice}</p>}</form>}</section>
      </div>
    </aside>}
