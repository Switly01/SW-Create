...
export function MemberHomePage() {
  ...
  return <main className="member-shell member-home-shell">
    ...
    <section className="member-content member-home-content"><div className="member-welcome"><p>{t.home} · SW IDENTITY BAĞLI</p><h1>{t.hello.toLocaleLowerCase(locale === "tr" ? "tr-TR" : "en-US")}, <em>{displayName}.</em></h1></div><div className="member-dashboard">
      ...
      <section className="member-whats-new"><div><span>SW SİNYALİ / v{SW_IDENTITY_VERSION}</span><h2>{t.news}</h2><p>SW Identity artık hızlı hesap seçimini, posta eklerini ve sağlayıcı bağlantılarını doğrulanmış veri akışında koruyor.</p><a className="member-updates-arrow" href="/updates/" aria-label="Güncelleme notlarını aç">GÜNCELLEME NOTLARINI AÇ <b>→</b></a></div><div className="member-release-list"><article><b>01</b><span>Canlı destek posta köprüsü<small>E-posta yanıtı ve güvenli ekleri açık konuşmaya sayfa yenilemeden döner.</small></span></article><article><b>02</b><span>Güvenli hesap seçimi<small>Parola saklanmadan, bu cihaza özel dönen anahtarla kayıtlı hesaba girilir.</small></span></article><article><b>03</b><span>Kimlik bağlantı ağı<small>Google ve Kick hesapları SW Identity merkezinden güvenle bağlanır.</small></span></article></div></section>
